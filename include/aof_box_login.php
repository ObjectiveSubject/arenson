<!-- mp duplicated box_login.php August 20, 2013 because of login error -->
...
ob_start() ?>
	...
	<div class="login_forms form_container clearfix <? echo $popup_method ?>"> 
		...
		<div class="close_window_button"><a href="#" class="close_fancybox" rel="login_form">Close</a></div>
		
		<div id="ajax_signin" class="login_subbox clearfix">
				<h4 class="login_access">Login to Access Your Account</h4>
			<form action="<?php bloginfo('url'); ?>/wp-login.php" method="post" id="memberloginform" name="memberloginform">				
				<label for="log" class="thecaption">Username</label>
				<input type="text" value="" title="Username"  tabindex="1" class="member_email member_info clearMeFocus" id="log" name="log" />
				<br />
				<label for="pwd" class="thecaption">Password<br /><a class="lost-pwd" rel="lostpasswdform" href="#lostpasswdform" >Forgot your password?</a></label>
				<input type="password"  tabindex="2" class="member_password member_info" id="pwd" name="pwd" />				
				<br />
				<label for="rememberme" class="rememberme thecaption">Remember me<br /><span class="explanation">Do not check if using a shared computer</span></label>
			...
			<div id="lostpasswdform" class="action_alerts">
				<form method="post" action="<?php echo site_url('wp-login.php?action=lostpassword', 'login_post') ?>" id="lostpasswordform" name="lostpasswordform">
					...
					<p class="submit clearfix"><?php do_action('login_form', 'resetpass'); ?><input type="submit" tabindex="100" value="Get New Password" class="aof_button gradient_blue" id="lostpass_submit" name="wp-submit"></p>
				</form>	
